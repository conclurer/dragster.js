...
import {MockDragonCloneElement} from './mock-dragon-clone-element';

export class MockDragster extends Dragster {
    public draggedElement: MockDragonElement | MockDragonCloneElement | null;
    ...
    protected triggerStart(context: IDragsterStartContext): void {
        ...
        if (this.requiresCopy(context.item, context.source)) {
            this.draggedElement = new MockDragonCloneElement(context.item, this.options, this);
        }
        else {
